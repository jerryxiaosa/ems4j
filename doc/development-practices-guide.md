...
## 缓存与序列化安全
- Redis 反序列化白名单为**可配置项**，默认不启用；如启用需配置 `redisson.allowlist-enabled=true`。
- 白名单包前缀通过 `redisson.allowlist-packages` 配置（如 `info.zhihui.ems`、`cn.dev33.satoken`、`java.util` 等）。
- 如需新增缓存类型，需评估是否在白名单范围内，避免引入不受控类型。
- 禁止在 Redis 中存储来自外部输入的“任意类型对象”，避免反序列化风险。
