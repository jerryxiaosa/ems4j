...
| 2 | 校验开户参数 | 账户是否已存在；企业业主是否存在；按账户类型校验参数（`MONTHLY` 必须有月租费，`QUANTITY` 必须有电价方案+预警方案，`MERGED` 按合并计费口径校验方案） | 抛业务异常，流程终止 |
| 3 | 创建账户主记录 | `MONTHLY` 清空账户电价方案；`QUANTITY` 清空月租费且不维护账户级预警等级；初始化预警状态为 `NONE` | 抛业务异常，事务回滚 |
...
### 5.5 余额变动监听分发（`BalanceChangedListener`）

账户模块在余额事件监听链路中的处理口径已按服务职责拆分，避免账户与电表后置逻辑耦合。

| 分发目标 | 处理范围 | 关键规则 | 失败处理 |
|------|------|------|------|
| `AccountBalanceChangeService` | 账户余额变动（`ACCOUNT`） | 仅对 `MONTHLY/MERGED` 生效；`QUANTITY` 跳过 | 记录告警并跳过，不抛错 |
| `MeterBalanceChangeService` | 电表余额变动（`ELECTRIC_METER`） | 仅处理电表余额后置逻辑，不承接账户余额逻辑 | 记录告警并跳过，不抛错 |

补充说明：

- 当前版本中，充值成功后不再直接触发电表自动开闸；自动开/关闸策略后续独立处理。
- `warnPlan` 缺失时统一记录告警并跳过相关后置处理，不中断充值与余额主流程。

### 5.6 账户类型字段规则（变更日期：2026-03-06）

本次调整的动机：统一账户侧与电表侧字段职责边界，避免不同计费类型出现重复配置和语义冲突。

| 账户类型 | 账户侧 `electricPricePlanId` | 账户侧 `warnPlanId` | 账户侧 `electricWarnType` | 电表侧 `price_plan_id` | 电表侧 `warn_plan_id` | 电表侧 `warn_type` |
|------|------|------|------|------|------|------|
| `QUANTITY` | 必填/可配置 | 必填/可配置 | 不维护 | 配置 | 配置 | 配置 |
| `MONTHLY` | 不允许配置（保持为空） | 允许配置 | 维护 | 不配置 | 不配置 | 不配置 |
| `MERGED` | 允许配置 | 允许配置 | 维护 | 不配置 | 不配置 | 不配置 |

补充说明：

- 电表侧规则保持不变：只有 `QUANTITY` 在开表时下发 `price_plan_id/warn_plan_id/warn_type`。
- 账户级预警等级（`electricWarnType`）仅适用于 `MONTHLY/MERGED`，`QUANTITY` 不再维护该字段。
