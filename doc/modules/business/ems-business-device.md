...
该流程由 `ElectricMeterManagerServiceImpl.setSwitchStatus` 驱动，采用“直接下发设备命令，成功后回写本地状态”的执行模型。

| 步骤 | 核心动作 | 关键校验/规则 | 失败处理 |
|------|----------|---------------|----------|
| 1 | 校验电表可操作性 | 电表必须存在、已同步 IoT、且在线 | 抛业务异常，流程终止 |
| 2 | 构建设备命令 | 按目标状态映射 `ENERGY_ELECTRIC_TURN_ON/OFF`，生成命令数据；该类命令不自动重试 | 参数不合法抛业务异常 |
| 3 | 保存并执行命令 | 业务层显式保存命令并执行下发，`saveDeviceCommand` 仅负责落库 | 执行失败抛业务异常 |
| 4 | 回写本地开关状态 | 仅在设备命令执行成功后更新 `is_cut_off` | 数据库更新异常抛出 |
| 5 | 记录操作日志 | 记录原状态、目标状态、命令信息 | 日志失败不影响主流程 |

补充说明：

- 无论数据库当前状态与目标状态是否一致，都会执行下发命令，避免设备状态漂移后无法纠正。
- 开闸/断闸命令不会自动重试，也不会在保存新命令前额外停用旧命令。
- 失败时可结合命令记录进行重试或人工排障。

### 5.2.1 新增电表初始化命令策略

- 新增电表采用“CT 同步保证、时间命令异步触发”的模型。
- 事务内依次完成：电表建档、IoT 同步、保存 CT 命令、同步执行 CT 命令。
- 如果 CT 执行失败，新增事务整体回滚，接口直接返回失败。
- CT 成功后，事务内继续保存尖峰平谷时间命令，并注册为事务提交后异步触发；该命令失败不影响当前新增接口返回。
- CT、开闸、断闸命令均不自动重试；尖峰平谷、指定日期电价命令允许自动重试。
- 该模型接受一个极低概率边界：CT 已在设备侧执行成功，但本地事务在提交阶段失败，可能出现跨系统不一致。

### 5.2.2 CT 变比修改执行模型

- `setMeterCt` 采用“事务内删旧建新 + 事务内最后同步执行 CT 命令”的模型。
- 事务内依次完成：校验、删旧表、建新表、保存 CT 命令、同步执行 CT 命令。
- CT 执行失败会直接让当前事务整体回滚，调用方同步感知失败。
- 该模型同样接受一个极低概率边界：CT 已在设备侧执行成功，但本地事务在提交阶段失败，可能出现跨系统不一致。
