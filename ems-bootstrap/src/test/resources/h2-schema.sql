...
CREATE INDEX idx_dcr_running_timeout ON device_command_record (is_deleted, is_running, last_execute_time);
...
(
    ...
    device_no          VARCHAR(50)      DEFAULT NULL COMMENT '电表编号',
...
(
    ...
    active_balance_key  VARCHAR(64) GENERATED ALWAYS AS (
        CASE
            WHEN is_deleted = FALSE THEN CAST(balance_relation_id AS VARCHAR) || '_' || CAST(balance_type AS VARCHAR)
            ELSE NULL END
        ),
    PRIMARY KEY (id)
);
...
(
    ...
    account_id          INTEGER COMMENT '账户id',
    owner_id            INTEGER COMMENT '账户归属者id',
    owner_type          SMALLINT COMMENT '账户类型，0企业1个人',
    owner_name          VARCHAR(50) COMMENT '企业/个人名称',
