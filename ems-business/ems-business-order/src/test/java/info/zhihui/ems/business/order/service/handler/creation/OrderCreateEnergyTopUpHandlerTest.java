...
class OrderCreateEnergyTopUpHandlerTest {
    ...
    void testHandle_WhenServiceRateIsZero_ShouldUseZeroServiceAmount() {
        ...
        verify(orderRepository).insert(orderCaptor.capture());
        OrderEntity capturedOrder = orderCaptor.getValue();
        assertEquals(new BigDecimal("0.00"), capturedOrder.getServiceAmount().setScale(2, RoundingMode.DOWN),
                "费率为0时服务费应为0");

        ArgumentCaptor<OrderDetailEnergyTopUpEntity> detailCaptor = ArgumentCaptor.forClass(OrderDetailEnergyTopUpEntity.class);
        verify(orderDetailEnergyTopUpRepository).insert(detailCaptor.capture());
        OrderDetailEnergyTopUpEntity capturedDetail = detailCaptor.getValue();
        assertEquals(new BigDecimal("100.00"), capturedDetail.getTopUpAmount().setScale(2, RoundingMode.DOWN),
                "费率为0时到账金额应等于订单金额");
    }
    ...
    void testHandle_WhenServiceRateIsPositiveAndFeeBelowOneCent_ShouldUseMinimumFee() {
        ...
        verify(orderRepository).insert(orderCaptor.capture());
        OrderEntity capturedOrder = orderCaptor.getValue();
        assertEquals(new BigDecimal("0.01"), capturedOrder.getServiceAmount().setScale(2, RoundingMode.DOWN),
                "费率大于0时服务费至少为0.01");

        ArgumentCaptor<OrderDetailEnergyTopUpEntity> detailCaptor = ArgumentCaptor.forClass(OrderDetailEnergyTopUpEntity.class);
        verify(orderDetailEnergyTopUpRepository).insert(detailCaptor.capture());
        OrderDetailEnergyTopUpEntity capturedDetail = detailCaptor.getValue();
        assertEquals(new BigDecimal("0.02"), capturedDetail.getTopUpAmount().setScale(2, RoundingMode.DOWN),
                "最小服务费生效后到账金额应正确");
    }
