...
public class DataPacketParser implements GatewayPacketParser {
    ...
    public AcrelMessage parse(ProtocolMessageContext context, byte[] payload) {
        ...
        if (gateway == null || !StringUtils.hasText(gateway.getDeviceSecret())) {
            log.warn("网关密钥缺失，无法解析数据，session={}", sessionId(context));
            return null;
        }
        try {
            ...
            if (report == null) {
                log.warn("网关数据解析失败，gateway={}", gateway.getDeviceNo());
                return null;
            }
            return new GatewayDataMessage(report, xml);
        } catch (Exception ex) {
            log.warn("网关数据解密失败，session={} gateway={} payloadSize={}",
