...
class DataUploadPacketDefinitionTest {
    ...
    void testDataUploadDefinition_DelegatesToParserAndHandler() {
        ...
        Mockito.when(parser.command()).thenReturn(AcrelPacketKeySupport.commandKey(Acrel4gCommandConstants.DATA_UPLOAD));
        Mockito.when(handler.command()).thenReturn(AcrelPacketKeySupport.commandKey(Acrel4gCommandConstants.DATA_UPLOAD));
        ...
        Assertions.assertEquals(AcrelPacketKeySupport.commandKey(Acrel4gCommandConstants.DATA_UPLOAD), definition.command());
