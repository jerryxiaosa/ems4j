...
class Acrel4gFrameCodecTest {
    ...
    void testEncode_NullBody_ShouldBuildFrameWithEmptyPayload() {
        ...
        byte command = Acrel4gCommandConstants.HEARTBEAT;
        byte[] frame = codec.encode(command, null);

        Assertions.assertEquals(7, frame.length);
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER, frame[0]);
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER, frame[1]);
        ...
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER_END, frame[5]);
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER_END, frame[6]);
    }

    @Test
    void testEncode_WithPayload_ShouldPlacePayloadAndCrc() {
        Acrel4gFrameCodec codec = new Acrel4gFrameCodec();
        byte command = Acrel4gCommandConstants.DOWNLINK;
        ...
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER_END, frame[8]);
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER_END, frame[9]);
    }

    @Test
    void testEncodeAck_ShouldMatchEncodeEmptyPayload() {
        Acrel4gFrameCodec codec = new Acrel4gFrameCodec();
        byte command = Acrel4gCommandConstants.REGISTER;
    ...
    void testEncodeTimeSync_ShouldEncodeTimeBody() {
        ...
        byte[] frame = codec.encode(Acrel4gCommandConstants.TIME_SYNC, buildTimeSyncBody(time));

        byte[] expectedBody = new byte[]{0x15, 0x0a, 0x15, 0x04, 0x13, 0x24, 0x2a};
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER, frame[0]);
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER, frame[1]);
        Assertions.assertEquals(Acrel4gCommandConstants.TIME_SYNC, frame[2]);
        Assertions.assertArrayEquals(expectedBody, Arrays.copyOfRange(frame, 3, 10));
        byte[] crcInput = new byte[1 + expectedBody.length];
        crcInput[0] = Acrel4gCommandConstants.TIME_SYNC;
        ...
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER_END, frame[12]);
        Assertions.assertEquals(Acrel4gFrameConstants.DELIMITER_END, frame[13]);
    }
