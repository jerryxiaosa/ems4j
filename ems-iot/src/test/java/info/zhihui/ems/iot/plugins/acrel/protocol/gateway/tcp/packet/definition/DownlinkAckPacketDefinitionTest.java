...
class DownlinkAckPacketDefinitionTest {
    ...
    void command_shouldReturnDownlinkAck() {
        ...
        Mockito.when(parser.command()).thenReturn(AcrelPacketKeySupport.commandKey(AcrelGatewayCommandConstants.DOWNLINK_ACK));
        Mockito.when(handler.command()).thenReturn(AcrelPacketKeySupport.commandKey(AcrelGatewayCommandConstants.DOWNLINK_ACK));
        DownlinkAckPacketDefinition definition = new DownlinkAckPacketDefinition(parser, handler);

        Assertions.assertEquals(AcrelPacketKeySupport.commandKey(AcrelGatewayCommandConstants.DOWNLINK_ACK), definition.command());
    }
