...
describe('useUserNotice', () => {
  ...
  test('testSetNotice_WhenTimerExpires_ShouldFadeAndClearNotice', () => {
    const { notice, noticeFading, setNotice } = useUserNotice({
      visibleMs: 1000,
      fadeMs: 200
    })
  ...
  test('testDispose_WhenTimerPending_ShouldPreventFollowupMutation', () => {
  ...
  test('testClearNoticeTimers_WhenClearTimerExists_ShouldStopClearStage', () => {
    const { notice, noticeFading, setNotice, clearNoticeTimers } = useUserNotice({
      visibleMs: 1000,
      fadeMs: 200
    })
  ...
  test('testSetNotice_WhenUseDefaultOptions_ShouldStillClearNotice', () => {
    const { notice, setNotice } = useUserNotice()
